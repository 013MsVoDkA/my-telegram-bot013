Готово! Вот полностью собранный код, где:

1. Ответы ограничены до **3–7 слов** (как на твоём любимом скрине).
2. Параметр `max_tokens` уменьшен до **40**, чтобы он физически не мог выдавать длинные тексты.
3. Полностью отключено цитирование и сохранён твой вайб.

Просто скопируй всё целиком в `bot.py` и деплой:
...
1. Пиши СУПЕР КОРОТКО. Максимум 3-7 слов в одном сообщении! Никаких длинных текстов и размышлений.
2. Отвечай легко, по-пацански, максимально просто. Без заумных фраз.
3. Помни контекст беседы, если она прислала голосовое — отвечай по сути.
4. Никаких эмодзи и смайликов.
5. Если хочешь отправить две короткие мысли подряд, разделяй их через |||

🚨 ОГРАНИЧЕНИЯ ДИСТАНЦИИ:
- Вы НЕ ВМЕСТЕ physically. Все действия (кофе, магазин, машина) делаешь только для себя. Не предлагай заехать за ней или взять ей что-то.
"""
...
- В ДНЕВНОЕ ВРЕМЯ: напиши краткую жизу ("Чем занимаешься, зай?", "Сделал кофе", "В машине сижу").
