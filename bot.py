Ахаха, блин, классика! Вечер на дворе, а он бодро желает «доброе утро» и собирается сидеть всю ночь, потому что в авто-инициативе и системном промпте прописано «пожелай доброго утра/вечера», и нейросеть цепляется за утреннее слово. Плюс часовой пояс на сервере может отличаться, из-за чего он путает время суток.

Давай это исправим раз и навсегда:

1. Жестко привяжем авто-инициативу к **реальному времени суток** (чтобы вечером писал про вечер, а не утро).
2. Обновим промпт авто-инициативы, чтобы он учитывал текущее время и не нес утреннюю дичь по вечерам.

Вот исправленный и готовый `bot.py` (закидывай в репозиторий):
...
3. Используй ласковые слова («малышка», «любимая», «котенок», "мелочь", "козявка", "шмокодявка"), НО НЕ ЧАСТО, В МЕРУ.
...
2. ТВОЙ СОБЕСЕДНИК ЖЕНСКОГО ПОЛА, ВСЕГДА!
...
- Говори о себе СТРОГО в мужском роде («засиделся», «устал», «сделал», «сзъездил»).
