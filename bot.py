    ...
    try:
        ...
        if FIL_STICKERS and should_send_sticker and random.random() < 0.20:
            try:
                ...
                if business_connection_id:
                    await context.bot.send_sticker(chat_id=chat_id, sticker=chosen_sticker, business_connection_id=business_connection_id)
                else:
                    await context.bot.send_sticker(chat_id=chat_id, sticker=chosen_sticker)
            except Exception as e:
...
async def handle_business_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"\n📥 [DEBUG] Поймано бизнес-сообщение от {chat_id}: {user_text}")
...
async def handle_normal_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = msg.chat.id
    user_text = msg.text
    print(f"\n📥 [DEBUG] Поймано обычное сообщение (личка): {chat_id} -> {user_text}")
...
async def handle_business_connection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.business_connection:
        print(f"\n🔗 BUSINESS CONNECTION: ID {update.business_connection.id}, enabled: {update.business_connection.is_enabled}")
...
if __name__ == "__main__":
    try:
        ...
        # Регистрируем точные обработчики для бизнеса и лички
        app.add_handler(TypeHandler(Update, handle_business_connection), group=-3)
        app.add_handler(MessageHandler(filters.BUSINESS_MESSAGE, handle_business_message), group=-2)
        app.add_handler(MessageHandler(filters.CHAT & ~filters.COMMAND & ~filters.BUSINESS_MESSAGE, handle_normal_message), group=-1)
        ...
        print("🚀 Бот запущен в режиме Поллинга с поддержкой Telegram Business...")
